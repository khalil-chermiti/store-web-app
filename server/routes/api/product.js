  ...
  async (req, res) => {
    try {
      ...
      const product = new Product({
        ...
        isActive,
        brand,
        imageUrl: image?.path ? host + image.path : "",
        imageKey: image?.path ? image.path : "",
      });
      ...
      res.status(400).json({
        success: false,
        message: "Votre demande n'a pas pu être traitée. Veuillez réessayer.",
      });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (foundProduct && foundProduct._id != productId) {
        return res
          .status(400)
          .json({ error: 'Le SKU ou le slug est déjà utilisé."' });
      }
