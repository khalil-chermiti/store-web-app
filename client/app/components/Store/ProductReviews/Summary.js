...
const Summary = props => {
  ...
  return (
    <div className='bg-white p-4 box-shadow-primary review-summary'>
      <h2 className='mb-0'>Évaluation</h2>
      {averageRating && (
        <div className='d-flex flex-wrap align-items-center mt-2'>
          <ReactStars
            ...
            value={averageRating}
          />
          {totalReviews > 0 && <span>basé sur {totalReviews} avis.</span>}
        </div>
      )}
      ...
      {totalReviews > 0 ? (
        ratingSummary.map((r, obj) => (
          <div key={obj} className='d-flex align-items-center mb-2'>
            <div className='left'>
              <span>{parseInt(Object.keys(r)[0])} étoile(s)</span>
            </div>
        ...
        <NotFound>
          ...
          <p className='mb-2'>Soyez le premier à ajouter un avis.</p>
        </NotFound>
      )}
    </div>
  );
